...
from .admin import ExceptionLogAdmin
...
class TestDbLogger(TestCase):
    def setUp(self):
        ...
        self.status_log_admin = ExceptionLogAdmin(ExceptionLog, AdminSite())

    def __test_log_aux(self, msg, fn, level):
        fn(msg)
        log_queryset = ExceptionLog.objects.filter(msg=msg)
    ...
    def test_exception(self):
        ...
        log_queryset = ExceptionLog.objects.filter(msg=exception_message)
        self.assertEqual(log_queryset.count(), 1)
        log = log_queryset.get()
